...
==============================================
Two detection backends selectable from the UI:

  DINO + SAM2  —  Grounding DINO for text-prompted bounding-box detection,
                  SAM 2 for mask segmentation.  Per-class thresholds and
                  multi-phrase prompts.  Manual Add clicks use SAM 2.

  SAM3         —  Meta's SAM 3.1 unified model (facebook/sam3.1).
                  One model handles text-prompted detection AND segmentation
                  in a single pass.  Manual Add clicks also use SAM3.
                  No DINO or SAM 2 needed.

Workflow:
  1. Choose backend, select models, click Load Selected Models.
     Active model is shown prominently in the status banner.
  2. Load image.
  3. Add class names — add descriptive phrases per class,
     set per-class thresholds (DINO+SAM2 only; ignored by SAM3).
  4. Run Detection.
...
Changes vs. v4
--------------
  * SAM3 backend  : select "SAM3" from the Backend dropdown.  Path defaults
    to C:\\hf_models\\sam3.1\\sam3.1_multiplex.pt (browsable).  SAM3 replaces
    both DINO and SAM2 — text-prompted detection and manual point clicks all
    go through the same model.
  * Prominent model status banner  : always visible below the Load button,
    shows backend name, model name(s), and device.  Updates immediately on
    load.  Ensures the user always knows which model is active during manual
    correction.
  * Backend switching hides/shows the relevant model rows so the UI stays
    uncluttered.

Changes vs. v3
--------------
  * Browse button for Grounding DINO model path.
  * Per-class detection phrases and phrase editor panel.
  * Per-class thresholds.
  * Menu bar: View, Tools (Merge COCO), Help.
  * Active-class selector (no per-click popup).

Requirements:
  pip install PyQt5 torch transformers accelerate pillow numpy torchvision
  pip install ultralytics   # for SAM3 backend
"""
...
# Default path for SAM3 weights — user can browse to a different location
SAM3_DEFAULT_PATH = rf"{MODEL_BASE}\sam3.1\sam3.1_multiplex.pt"
...
class SAM3ModelLoader(QThread):
    """Loads SAM3 via the Ultralytics API."""
    finished = pyqtSignal(object)   # emits the predictor
    progress = pyqtSignal(str)
    error    = pyqtSignal(str)
    ...
    def run(self):
        try:
            ...
            self.progress.emit("Loading SAM3...")
...
class SAM3DetectionWorker(QThread):
    """
    Runs SAM3 text-prompted detection for each class and returns results
    in the same dict format as DetectionWorker so _on_det_done needs no changes.

    SAM3 returns masks directly from a text prompt — no separate detection
    and segmentation steps.  One pass per class using all that class's phrases.
    """
    finished = pyqtSignal(list)
    progress = pyqtSignal(str)
    error    = pyqtSignal(str)

    def __init__(self, image_pil, class_configs, sam3_predictor):
        super().__init__()
        self.image         = image_pil
        self.class_configs = class_configs
    ...
    def run(self):
        try:
            ...
            # Write image to a temp file so SAM3 gets a file path, which is
            # exactly how the confirmed-working standalone usage works:
            #   predictor.set_image("image.jpg")
            #   results = predictor(text=["glomerulus"])
            ...
            for cfg in self.class_configs:
                ...
                for phrase in phrases:
                    self.progress.emit(
                        f'SAM3: "{cfg["name"]}" — prompt: "{phrase}"')
                    try:
                        self.predictor.set_image(tmp.name)
                        preds = self.predictor(text=[phrase])
                    except Exception as e:
                        self.progress.emit(f'  warning: {e}')
                        continue

                    if preds is None:
                        continue

                    # preds is a list; each element has .masks.data (N, H, W)
                    for pred in preds:
                        if pred.masks is None:
                            continue
                        masks_data = pred.masks.data.cpu().numpy()  # (N, H, W)
                        # Scores: use pred.boxes.conf if available, else 1.0
                        if pred.boxes is not None and len(pred.boxes.conf):
                            confs = pred.boxes.conf.cpu().numpy()
                        else:
                            confs = [1.0] * len(masks_data)

                        for mask_arr, conf in zip(masks_data, confs):
                            mask_bool = mask_arr.astype(bool)
                            if not mask_bool.any():
                                continue
                            # Skip masks covering most of the image
                            ih, iw = mask_bool.shape
                            if mask_bool.sum() / (ih * iw) > MAX_AREA_FRAC:
                                continue
                            results.append({
                                "class_name": cfg["name"],
                                "mask":       mask_bool,
                                "score":      float(conf),
                                "iou":        1.0,   # SAM3 does not expose IoU
                                "source":     "llm",
                            })

            os.unlink(tmp.name)
            self.progress.emit(
                f"SAM3 detection done: {len(results)} mask(s).")
            self.finished.emit(results)
...
def segment_point_sam3(image_pil, x, y, sam3_predictor):
    """
    SAM3 point-prompted segmentation for manual Add clicks.
    Returns (mask_bool, iou_float) — same signature as segment_point.

    Uses a temp file path so set_image matches the confirmed-working
    standalone usage. The file is kept alive until after inference completes.
    Points are passed as a numpy array (1, 1, 2) as Ultralytics expects.
    """
    import numpy as _np
    import tempfile, os
    ...
    try:
        sam3_predictor.set_image(tmp.name)
        points = _np.array([[[x, y]]], dtype=_np.float32)
        labels = _np.array([[1]],      dtype=_np.int32)
        preds = sam3_predictor(points=points, labels=labels)
    finally:
        os.unlink(tmp.name)

    if not preds or preds[0].masks is None:
        ih, iw = image_pil.size[1], image_pil.size[0]
        return _np.zeros((ih, iw), dtype=bool), 0.0

    masks_data = preds[0].masks.data.cpu().numpy()
    if preds[0].boxes is not None and len(preds[0].boxes.conf):
        confs = preds[0].boxes.conf.cpu().numpy()
        best  = int(_np.argmax(confs))
    else:
        best = 0

    mask = masks_data[best].astype(bool)
    return mask, 1.0
...
class AnnotationTool(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(
            "LLM-Assisted Annotation Tool  |  DigitalSreeni  |  v5")
    ...
    def _build_ui(self):
        ...
        self.combo_backend.addItems(["DINO + SAM2", "SAM3"])
        self.combo_backend.currentTextChanged.connect(self._on_backend_changed)
        backend_row.addWidget(self.combo_backend, 1)
        mdl_layout.addLayout(backend_row)

        sep_backend = QFrame()
        sep_backend.setFrameShape(QFrame.HLine)
        sep_backend.setStyleSheet("color:#ddd;margin:2px 0;")
        mdl_layout.addWidget(sep_backend)

        # ---- DINO + SAM2 rows (shown when backend = DINO+SAM2) ----------
        self._dino_sam2_widget = QWidget()
        dino_sam2_layout = QVBoxLayout(self._dino_sam2_widget)
        dino_sam2_layout.setContentsMargins(0, 0, 0, 0)
        dino_sam2_layout.setSpacing(4)

        dino_sam2_layout.addWidget(QLabel("Grounding DINO:"))
        ...
        dino_sam2_layout.addWidget(self.combo_gdino)

        # Browse row for custom DINO
        ...
        dino_sam2_layout.addWidget(self._gdino_browse_row)

        browse_note = QLabel(
            "To use a fine-tuned model: select 'Custom / fine-tuned', "
            "browse to best_checkpoint/ or final_checkpoint/.")
        browse_note.setWordWrap(True)
        browse_note.setStyleSheet("font-size:10px;color:#777;font-style:italic;")
        dino_sam2_layout.addWidget(browse_note)

        dino_sam2_layout.addWidget(QLabel("SAM 2:"))
        self.combo_sam2 = QComboBox()
        for k in SAM2_MODELS:
            self.combo_sam2.addItem(k)
        dino_sam2_layout.addWidget(self.combo_sam2)

        mdl_layout.addWidget(self._dino_sam2_widget)

        # ---- SAM3 row (shown when backend = SAM3) -----------------------
        self._sam3_widget = QWidget()
        sam3_layout = QVBoxLayout(self._sam3_widget)
        sam3_layout.setContentsMargins(0, 0, 0, 0)
        sam3_layout.setSpacing(4)

        sam3_layout.addWidget(QLabel("SAM3 weights (.pt):"))
        sam3_path_row = QHBoxLayout()
        self.lbl_sam3_path = QLabel(SAM3_DEFAULT_PATH)
        self.lbl_sam3_path.setWordWrap(True)
        self.lbl_sam3_path.setStyleSheet("font-size:10px;color:#555;")
        btn_browse_sam3 = QPushButton("Browse")
        btn_browse_sam3.setFixedWidth(60)
        btn_browse_sam3.clicked.connect(self._browse_sam3_model)
        sam3_path_row.addWidget(self.lbl_sam3_path, 1)
        sam3_path_row.addWidget(btn_browse_sam3)
        sam3_layout.addLayout(sam3_path_row)

        sam3_note = QLabel(
            "SAM3 handles text-prompted detection and manual clicks — "
            "no DINO or SAM2 needed.")
        sam3_note.setWordWrap(True)
        sam3_note.setStyleSheet("font-size:10px;color:#777;font-style:italic;")
        sam3_layout.addWidget(sam3_note)

        self._sam3_widget.setVisible(False)
        mdl_layout.addWidget(self._sam3_widget)
        ...
        self.lbl_model_status.setMinimumHeight(48)
        ...
        note = QLabel(
            "DINO+SAM2: one DINO pass per class, then cross-class NMS "
            "(IoU >= {:.2f}), then SAM2 segmentation.\n"
            "SAM3: one text-prompted pass per class phrase directly "
            "producing masks.".format(CROSS_CLASS_NMS_THR))
        ...
        hint = QLabel(
            ...
            "Manual clicks use whichever model is loaded (see banner above).")
        hint.setWordWrap(True)
        hint.setStyleSheet("font-size: 10px; color: #777; font-style: italic;")
    ...
    def _show_about(self):
        QMessageBox.about(
            ...
            "<b>LLM-Assisted Scientific Image Annotation Tool</b> v5<br>"
            ...
            "Two detection backends:<br>"
            "<b>DINO + SAM2</b> — Grounding DINO text-prompted detection "
            "with SAM 2 segmentation. Supports per-class thresholds, "
            "multi-phrase prompts, and fine-tuned DINO checkpoints.<br><br>"
            "<b>SAM3</b> — Meta's SAM 3.1 unified model. One model handles "
            "text-prompted detection and segmentation, and manual point clicks. "
            "No DINO or SAM2 needed.<br><br>"
    ...
    def _on_backend_changed(self, text):
        is_sam3 = (text == "SAM3")
        self._dino_sam2_widget.setVisible(not is_sam3)
        self._sam3_widget.setVisible(is_sam3)
        # SAM3 is a semantic model — point-click Add mask is not reliable.
        # Disable the Add radio button and switch to View if it was selected.
        self.rb_add.setEnabled(not is_sam3)
        if is_sam3 and self.mode_group.checkedId() == 1:
            self.rb_view.setChecked(True)
        self.rb_add.setToolTip(
            "Not available in SAM3 mode — SAM3 does not support point-click segmentation."
            if is_sam3 else
            "Click a missed object to add its mask using SAM2.")
        self._mark_reload_needed()

    def _browse_sam3_model(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select SAM3 weights file", MODEL_BASE,
            "PyTorch weights (*.pt *.pth)")
        if path:
            self.lbl_sam3_path.setText(path)
            self._mark_reload_needed()
    ...
    def _load_models(self):
        ...
        if backend == "SAM3":
            model_path = self.lbl_sam3_path.text().strip()
            if not model_path:
                QMessageBox.warning(
                    self, "No model path",
                    "Browse to the SAM3 .pt weights file before loading.")
                return
            self.btn_load_models.setEnabled(False)
            self.btn_detect.setEnabled(False)
            self.models_ready = False
            self.lbl_model_status.setText("Loading SAM3...")
            self.lbl_model_status.setStyleSheet(
                "color:#888;font-size:12px;font-weight:bold;"
                "background:#f0f0f0;padding:6px;border-radius:4px;"
                "border:1px solid #ccc;")
            self.status.showMessage("Loading SAM3 — this may take a moment...")
            self.loader = SAM3ModelLoader(model_path)
            self.loader.progress.connect(self._on_mdl_progress)
            self.loader.finished.connect(self._on_sam3_mdl_ready)
            ...
            self.btn_detect.setEnabled(False)
            self.models_ready = False
            self.lbl_model_status.setText("Loading DINO + SAM2...")
            self.lbl_model_status.setStyleSheet(
                "color:#888;font-size:12px;font-weight:bold;"
                "background:#f0f0f0;padding:6px;border-radius:4px;"
                "border:1px solid #ccc;")
    ...
    def _on_mdl_ready(self, gp, gm, sp, sm):
        ...
        self.sam2_proc      = sp
        self.sam2_model     = sm
        self.sam3_predictor = None   # clear any previous SAM3
        ...
        sam2_display = self.combo_sam2.currentText().split("(")[0].strip()

        banner = (f"DINO + SAM2  |  {DEVICE.upper()}\n"
                  f"DINO: {gdino_display}\n"
                  f"SAM2: {sam2_display}")
        self.lbl_model_status.setText(banner)
        self.lbl_model_status.setStyleSheet(
            "color:#1a5490;font-size:12px;font-weight:bold;"
            "background:#ddeeff;padding:6px;border-radius:4px;"
            "border:1px solid #7aadd4;")

        if self.image_pil is not None:
            self.btn_detect.setEnabled(True)
        self.status.showMessage(
            f"DINO + SAM2 ready on {DEVICE.upper()}. "
            "Load an image or run detection.")

    def _on_sam3_mdl_ready(self, predictor):
        self.sam3_predictor = predictor
        self.gdino_proc     = None   # clear any previous DINO+SAM2
        self.gdino_model    = None
        self.sam2_proc      = None
        self.sam2_model     = None
        self.models_ready   = True
        ...
        from pathlib import Path as _Path
        model_name = _Path(self.lbl_sam3_path.text()).name

        banner = f"SAM3  |  {DEVICE.upper()}\n{model_name}"
        self.lbl_model_status.setText(banner)
        self.lbl_model_status.setStyleSheet(
            "color:#7d3c98;font-size:12px;font-weight:bold;"
            "background:#f5eef8;padding:6px;border-radius:4px;"
            "border:1px solid #c39bd3;")

        if self.image_pil is not None:
            self.btn_detect.setEnabled(True)
        self.status.showMessage(
            f"SAM3 ready on {DEVICE.upper()}. "
            "Load an image or run detection.")
    ...
    def _run_detection(self):
        ...
        if not self.models_ready:
            QMessageBox.warning(self, "Models Not Ready", "Load models first.")
            return
        ...
        self.status.showMessage(f'Running detection for: {names_str}')

        backend = self.combo_backend.currentText()

        if backend == "SAM3":
            self.det_worker = SAM3DetectionWorker(
                self.image_pil, class_configs, self.sam3_predictor)
        else:
    ...
    def _add_mask_at(self, ix, iy):
        ...
        try:
            backend = self.combo_backend.currentText()
            if backend == "SAM3":
                mask, iou = segment_point_sam3(
                    self.image_pil, ix, iy, self.sam3_predictor)
            else:
            ...
            model_tag = "SAM3" if backend == "SAM3" else "SAM2"
            self.status.showMessage(
                f"Added manual mask for '{chosen}'  "
                f"[{model_tag}]  IoU={iou:.2f}.  "
                f"Total: {len(self.annotations)}")
        except Exception as e:
